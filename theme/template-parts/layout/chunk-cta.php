...
<section class="<?php echo esc_attr( $args['class'] ); ?> cta | full-bleed ll-equal-vert-padding not-prose bg-brand-blue text-neutral-50 break-inside-avoid print:hidden">
	...
	<div class="px-2 z-10  |  lg:px-[16px]">
		<div class="flex flex-col items-start gap-8  |  sm:flex-row sm:items-center lg:gap-24">
			...
			<div class="prose grow  |  lg:prose-xl ">
				<h2 class="mb-2 text-orient-200 text-shadow shadow-brand-blue-dark  |  print:text-shadow-none">
					<?php echo esc_html( $args['part_data']['cta_heading'] ); ?>
				</h2>
				<?php if ( $args['part_data']['cta_body'] ) :
					echo '<p class="text-neutral-100 text-shadow shadow-brand-blue-dark  |  print:text-shadow-none">' . esc_html( $args['part_data']['cta_body'] ) . '</p>';
				endif; ?>
				<p class="hidden  |  print:mt-8 print:block">Email info@beachfleischman.com</p>
			</div>

			<div class="w-full  |  md:max-w-fit">
				<a href="<?php echo esc_html( $args['part_data']['cta_button_url'] ); ?>" class="px-5 py-4 font-head font-semibold border-2 border-brand-blue-dark bg-brand-blue-dark rounded-lg text-orient-200  |  hover:text-white hover:border-orient-200">
