...
<article id="post-<?php the_ID(); ?>" <?php post_class( 'bg-neutral-100 rounded-2xl group' ); ?> data-tilt data-tilt-reverse="true">
	...
	<div class="card-inner | p-4 bg-center rounded-2xl grayscale group-hover:grayscale-0 group-hover:bg-blend-multiply" style="background-image: url('<?php echo $feat_image_url[0]; ?>'), linear-gradient(to bottom, #a3a3a3 0%, #282828 100%)">
		<a class="flex flex-col place-content-between w-[240px] h-auto aspect-square p-4 " href="<?php echo esc_url( get_permalink( get_the_ID() ) ); ?>" rel="bookmark">
			<?php echo '<div class="text-center"><i class="fa-regular ' . $icon . ' fa-2x lg:fa-4x text-white group-hover:text-brand-red" title="' . get_the_title() . '"></i></div>'; ?>
